...
create_client() {
  local cid=$1
  local url=$2
  local id=$(${KCADM} create clients -r ${REALM} -s clientId=${cid} -s enabled=true -s protocol=openid-connect -s publicClient=false -s 'redirectUris=["'${url}'/*"]' -s 'webOrigins=["*"]' -i)
  ${KCADM} update clients/${id}/client-secret -r ${REALM} -s value=${CLIENT_SECRET}
}

create_client portal-issuer ${ISSUER_URL}
create_client portal-investor ${INVESTOR_URL}
create_client backoffice ${BACKOFFICE_URL}
